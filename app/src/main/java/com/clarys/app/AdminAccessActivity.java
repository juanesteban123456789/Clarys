...
public class AdminAccessActivity extends BaseScreenActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        View googleButton = findViewById(R.id.buttonGoogleLogin);
        View registerButton = findViewById(R.id.buttonRegisterAdmin);
    ...
    private void signIn() {
        ...
        if (email.isEmpty() || password.isEmpty()) {
            showMessage("Escribe correo y contraseña");
            return;
        }
        if (!ValidationUtils.isValidEmail(email)) {
            showMessage("Escribe un correo válido");
            return;
        }
        if (!ValidationUtils.hasReasonableTextLength(password, ValidationUtils.MAX_SHORT_TEXT_LENGTH)) {
            showMessage("La contraseña es demasiado larga");
            return;
        }
    ...
    private void showRegisterOptions() {
        new AlertDialog.Builder(this)
                ...
                .setItems(new String[]{"Con correo y contraseña", "Con Google"}, (dialog, which) -> {
                    if (which == 0) {
    ...
    private void showEmailRegisterDialog() {
        ...
        EditText password = addDialogInput(form, "Contraseña", InputType.TYPE_CLASS_TEXT
        ...
        dialog.setOnShowListener(shownDialog -> dialog.getButton(AlertDialog.BUTTON_POSITIVE)
                .setOnClickListener(view -> {
                    if (registerWithEmail(email, password, workshop, country, whatsapp)) {
                        dialog.dismiss();
                    }
                }));
        dialog.show();
    ...
    private boolean registerWithEmail(EditText email, EditText password, EditText workshop, Spinner country,
            EditText whatsapp) {
        String emailValue = email.getText().toString().trim();
        String passwordValue = password.getText().toString();
        String workshopValue = workshop.getText().toString().trim();
        String whatsappValue = buildWhatsapp(country, whatsapp);
        ...
        if (passwordValue.length() < 6 || passwordValue.length() > ValidationUtils.MAX_SHORT_TEXT_LENGTH) {
            showMessage("La contraseña debe tener entre 6 y 80 caracteres");
            return false;
        }
        ...
        if (!ValidationUtils.hasReasonableTextLength(workshopValue, ValidationUtils.MAX_SHORT_TEXT_LENGTH)) {
            showMessage("El nombre del taller es demasiado largo");
            return false;
        }
        if (!ValidationUtils.isValidPhone(whatsappValue)) {
            showMessage("Escribe un WhatsApp válido");
            return false;
        }

        store.signUpAdmin(emailValue, passwordValue, workshopValue, whatsappValue,
    ...
    private void showGoogleRegisterDialog() {
        ...
        dialog.setOnShowListener(shownDialog -> dialog.getButton(AlertDialog.BUTTON_POSITIVE)
                .setOnClickListener(view -> {
                    if (startGoogleRegisterFlow(workshop.getText().toString(), buildWhatsapp(country, whatsapp))) {
                        dialog.dismiss();
                    }
                }));
        dialog.show();
    ...
    private boolean startGoogleRegisterFlow(String workshopName, String whatsapp) {
        String workshopValue = workshopName.trim();
        if (workshopValue.isEmpty()) {
            showMessage("El nombre del taller es obligatorio");
            return false;
        }
        ...
        startGoogleFlow(true, workshopValue, whatsapp);
        return true;
    }
    ...
    private Spinner addCountrySpinner(LinearLayout form) {
        ...
        label.setText("País del WhatsApp");
    ...
    private String buildWhatsapp(Spinner country, EditText whatsapp) {
        ...
        String rawPhone = ValidationUtils.digitsOnly(whatsapp.getText().toString());
        if (rawPhone.isEmpty()) {
            return "";
        }
    ...
    private void handleGoogleResult(Intent data) {
        ...
        try {
            ...
            if (idToken == null || idToken.trim().isEmpty()) {
                showMessage("Google no entregó el token de acceso");
                return;
            }
